...
amplitude_low = np.array((CD_low/2))
amplitude_med = np.array((CD_med/2))
amplitude_hi = np.array((CD_hi/2))
...
#Adding the fact that all of these graphs have a point at the origin. This is likely to give a more accurate
#estimate for our poynomial fiiting on the points.

amplitude_low_curve = np.concatenate([[0], amplitude_low])
amplitude_med_curve = np.concatenate([[0], amplitude_med])
amplitude_hi_curve = np.concatenate([[0], amplitude_hi])
omega_low_curve = np.concatenate([[0], omega_low])
omega_med_curve = np.concatenate([[0], omega_med])
omega_hi_curve = np.concatenate([[0], omega_hi])
...
#Defining Function for the Phase difference of the torsion pendulum against the 
#angular frequency.
    
#for this approximation we will be using a Maclaurin series expansion of the equation
#up to 4 terms.

def phase(x, const_lambda, mom_inertia, torsion_k):
    return (- x*(const_lambda / torsion_k)) + ((2*mom_inertia*((const_lambda**2) - (3*torsion_k*mom_inertia)))/(torsion_k**3))
...
params_amp_low, params_amp_low_covariance = optimize.curve_fit(amplitude, omega_low_curve, amplitude_low_curve)
params_amp_med, params_amp_med_covariance = optimize.curve_fit(amplitude, omega_med_curve, amplitude_med_curve)
params_amp_hi, params_amp_hi_covariance = optimize.curve_fit(amplitude, omega_hi_curve, amplitude_hi_curve)
...
#Using scipy's curve fit function in order to find out the most appropraite values
#for our variables in the context of our phase function.
params_phase_low, params_phase_low_covariance = optimize.curve_fit(phase, omega_low, phi_low)
params_phase_med, params_phase_med_covariance = optimize.curve_fit(phase, omega_med, phi_med)
params_phase_hi, params_phase_hi_covariance = optimize.curve_fit(phase, omega_hi, phi_hi)
...
plt.figure(2)
plt.plot(x, phase(x, params_phase_low[0], params_phase_low[1], params_phase_low[2]), label="Low", marker="x")
plt.plot(x, phase(x, params_phase_med[0], params_phase_med[1], params_phase_med[2]), label="Medium",  marker="x")
plt.plot(x, phase(x, params_phase_hi[0], params_phase_hi[1], params_phase_hi[2]),  label="High",  marker="x")
